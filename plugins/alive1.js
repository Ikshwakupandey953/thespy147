...
Asena.addCommand({pattern: 'alive', fromMe: false, desc: Lang.ALIVE_DESC}, (async (message, match) => {
        let caption = '```Jarvis is Alive Sir!!!!```\n\n*Version:* ```' + Config.VERSION + '```\n*Branch:* ```' + Config.BRANCH + '```\n\n```Type``` *.sam* ```for command list``` \n\n ```Coded By``` *Sam Pandey*\n'
        let pp
...
Asena.addCommand({pattern: 'sysda', fromMe: false, desc: Lang.SYSD_DESC}, (async (message, match) => {
...
Asena.addCommand({pattern: 'palive', fromMe: true, dontAddCommandList: true }, (async (message, match) => {
    if (Config.ALIVEMSG == 'default') {
        await message.client.sendMessage(message.jid,'```Sam Pandey```\n\n*Version:* ```'+Config.VERSION+'```\n*Branch:* ```'+Config.BRANCH+'```' , MessageType.text);
    }
    else {
